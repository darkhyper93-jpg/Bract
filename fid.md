...
> Agente I (quiz) COMPLETO y deployado · I-2 (progreso/personalización): spec + plan APROBADOS, ejecución NO arrancada · Retomar en: repo Bract (darkhyper93-jpg/Bract)
...
**PUNTO DE RETOMA — I-2 (Progreso, puntos débiles y personalización):** spec §3.6 del README + plan de implementación (`docs/superpowers/plans/2026-06-14-i2-progress-personalization.md`) escritos y APROBADOS por el orquestador, commiteados en la branch **`agente-i2-progress`** (pusheada a GitHub, NO mergeada, main intacto). Falta SOLO ejecutar el código (F1→F7). Próximo mensaje al agente: "arrancá F1, modo subagent-driven, pará en bordes de fase con git diff --stat + checks verdes, revisión con lupa en F2/F5/F6".
- Diseño aprobado: 3 capas. Capa 1 motor (`progress.formula.ts` puro + `progress.repository.ts` groupBy sin N+1 + `progress.service.ts` getOverview/getWeakTopics/getWeaknessMap) + dashboard sección nueva "Progreso" (/progress). Capa 2 planner aditivo (GeneratePlanInput suma topics[].weakness? + remediationAlpha?; solo cambia el .sort() de buildBaselinePlan; effectiveDays = examDays − α·D·weakness; en el grupo SIN examen ordena también por weakness — ajuste pedido). Capa 3 chat aditivo (StudentContext.weakTopics? opcional, render condicional). Personalización: model nuevo `UserStudyPreferences` (enum RemediationIntensity OFF/LOW/MEDIUM/HIGH default LOW; α 0/0.33/0.66/1.0; prioritySubjectIds[]; pesos). Degradación try/catch → sin datos/OFF/error = byte-idéntico a hoy; 2 golden tests.
- **Almacenamiento on-the-fly** (groupBy sobre índices existentes, sin tabla de caché). **Único `db push` = `UserStudyPreferences`** (Task 10): cuando el agente llegue ahí PARA y el usuario corre el comando con Session pooler 5432 (NO el comando genérico del plan).
- Pendiente roadmap tras I-2: L Voz (Web Speech API) → J Gamificación (refs codex.io/Arise/coddy.tech) → pase estético premium (ref codex.io).
