...
- [ ] **F1 — Shared:** `types/gamification.types.ts` (`GamificationSummary`, `DailyQuest`, `DailyBoss`, enums) + `lib/gamification.xp.ts` puro (`levelForXp` + constantes de XP/curva) reexportado; `typecheck` verde + rebuild de `dist`. Sin `schema.ts` (el único endpoint es un GET sin input del cliente → nada que validar con Zod)
