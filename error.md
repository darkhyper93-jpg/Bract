...
- **Abort real del proveedor en disconnect:** `gen.return()` interrumpe el `for await (delta of streamChatReply(...))`, que propaga `return()` al stream del SDK de Anthropic → aborta el request HTTP → no se siguen gastando tokens. No se modificó B (no se le pasa `AbortSignal`): alcanza con la propagación de `return()` del generador.
