...
export function buildChatSystemPrompt(contextText: string): string {
  return [
    ...
    'Podés usar listas SOLO cuando de verdad estás enumerando varios elementos. En ese caso: una viñeta por ítem empezando con el carácter "·" (punto medio), y un renglón en blanco entre un ítem y el siguiente para que respire (estilo Canva: limpio y que resalte). NUNCA uses "-", "*" ni números (1. 2. 3.) para las listas; siempre el "·".',
